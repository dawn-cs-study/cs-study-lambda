package com.dawn.cs.study.lambda.sync.handler;

import com.amazonaws.services.lambda.runtime.events.S3Event;
import com.amazonaws.services.lambda.runtime.events.models.s3.S3EventNotification.S3EventNotificationRecord;
import com.dawn.cs.study.lambda.sync.application.DeleteMarkdownEmbeddingUseCase;
import com.dawn.cs.study.lambda.sync.application.IngestMarkdownToVectorStoreUseCase;
...
public class S3MarkdownVectorSyncHandler implements Function<S3Event, String> {

    private final IngestMarkdownToVectorStoreUseCase ingestMarkdownToVectorStoreUseCase;

    private final DeleteMarkdownEmbeddingUseCase deleteMarkdownEmbeddingUseCase;
    ...
    private Runnable buildTask(S3EventNotificationRecord r) {
        ...
        return switch (eventName.split(":")[0]) { // "ObjectCreated" or "ObjectRemoved"
            case "ObjectRemoved" -> switch (getExtension(key)) {
                case "md" -> () -> deleteMarkdownEmbeddingUseCase.deleteMarkdownEmbedding(key);
                default -> throw new RuntimeException(String.format("잘못된 파일 확장자입니다. 확장자는 반드시 'md' 이어야 합니다. %s", key));
            };
            case "ObjectCreated" -> switch (getExtension(key)) {
                case "md" -> () -> ingestMarkdownToVectorStoreUseCase.markdownEmbedding(key);
                default -> throw new RuntimeException(String.format("잘못된 파일 확장자입니다. 확장자는 반드시 'md' 이어야 합니다. %s", key));
            };
